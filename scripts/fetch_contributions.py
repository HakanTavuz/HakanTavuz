#!/usr/bin/env python3
"""GitHub'ın public contribution HTML'ini çek, token yok."""
...
from config import resolve_username  # noqa: E402
...
def fetch_days(username: str) -> list[dict]:
    ...
    if not cells:
        print("takvim hücresi yok — GitHub markup değişmiş olabilir", file=sys.stderr)
        sys.exit(1)
    ...
    for td in cells:
        ...
        if raw_count is not None and str(raw_count).isdigit():
            ...
            if re.search(r"no contributions", text, re.I):
                ...
                m = re.match(r"(\d+)", text)
                count = int(m.group(1)) if m else int(td.get("data-level") or 0)
...
def compute_current_streak(days: list[dict]) -> tuple[int, str | None, str | None]:
    idx = len(days) - 1
    if days[idx]["count"] == 0:
        idx -= 1
    streak = 0
...
def main() -> None:
    ...
    if username in {"", "YOUR_USERNAME"}:
        raise SystemExit(
            "GH_PROFILE_USER veya scripts/config.py içindeki USERNAME'i gerçek GitHub adınla doldur."
        )
